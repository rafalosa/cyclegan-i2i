...
def _unpack_dataset(path_raw: str, path_unpack: str, verbose=True):
    if verbose:
...
def get_maps_dataset(target_directory: str, verbose=True):
    ...
    if dataset_temp_name not in os.listdir(dataset_temp_dir):
        ...
        wget.download(dataset_url, f"{dataset_temp_dir}/{dataset_temp_name}", bar=_progress_bar)
        if verbose:
            print("")
    ...
    if dataset_dir not in os.listdir():
        ...
        _unpack_dataset(f"{dataset_temp_dir}/{dataset_temp_name}", dataset_dir, verbose=verbose)
    else:
